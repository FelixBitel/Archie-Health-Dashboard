...
const ArchieDashboard = () => {
      ...
      const response = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: 'user',
              content: `Ты ветеринарный помощник для собаки по кличке Арчи. Данные: ${JSON.stringify(archieData)}. Вопрос: ${currentInput}`
            }
          ]
        })
      });
  ...
  const handleMealIngredientChange = (index, value) => {
    setNewMeal(prev => {
  ...
  const handleMealCaloriesChange = (e) => {
    setNewMeal(prev => ({ ...prev, calories: parseInt(e.target.value) || 0 }));
  };
  ...
  const handleRecipeIngredientChange = (index, value) => {
    setNewRecipe(prev => {
      const newIngs = [...prev.ingredients];
      newIngs[index] = value;
      return { ...prev, ingredients: newIngs };
  ...
  const handleRecipeCaloriesChange = (e) => {
    setNewRecipe(prev => ({ ...prev, calories: parseInt(e.target.value) || 0 }));
  };

  const handleRecipePortionsChange = (e) => {
    setNewRecipe(prev => ({ ...prev, portions: parseInt(e.target.value) || 1 }));
  };
          ...
          <div className="bg-white rounded-xl p-6 max-w-md w-full">
            ...
            {newMeal.ingredients.map((ing, i) => (
              <input
                ...
                value={ing}
                onChange={(e) => handleMealIngredientChange(i, e.target.value)}
                className="w-full border rounded-lg p-2 mb-2"
              />
            ))}
            <button
              onClick={() => setNewMeal(prev => ({ ...prev, ingredients: [...prev.ingredients, ''] }))}
            ...
            <input
              ...
              onChange={handleMealCaloriesChange}
          ...
          <div className="bg-white rounded-xl p-6 max-w-md w-full">
            ...
            {newRecipe.ingredients.map((ing, i) => (
              <input
                ...
                placeholder={`Ингредиент ${i + 1}`}
                value={ing}
                onChange={(e) => handleRecipeIngredientChange(i, e.target.value)}
                className="w-full border rounded-lg p-2 mb-2"
              />
            ))}
            <button
            ...
            <input
              ...
              onChange={handleRecipePortionsChange}
              className="w-full border rounded-lg p-2 mb-4"
            />
            <div className="flex gap-2">
